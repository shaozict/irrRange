function IrrRange(parm) {
	this.parm = $.extend({
		parElement: ".shouFu_range",
	}, parm);
	this.$display = $("#showShouFu");
	this.$range = $("#shouFuRange");
	this.$slect = $("#shouFuRange").find(".rangeSlect");
}
IrrRange.prototype = {
	getdata: function() {
		var $li = $this.find("ul").eq(0).find("li");
		//获取所有独立区间左值 
		//获取所有独立区间左值
		vis = {
			left: $li.offsetLefts(),
	...
	range: function(tar) {
		...
		$target.on({
			...
			"changeOffset": function(event, pClientX) {
				...
				slectFrontX = Range.spanvalue(vis.min, vis.max)(slectlongX.x1);
				slectBackX = Range.spanvalue(vis.min, vis.max)(slectlongX.x2);
				var xFobj = Range.floorArr(vis.left, slectFrontX); //获取数组rangeAxisLiOffsetLefts中小于xFm的最大元素值
				var xBobj = Range.floorArr(vis.left, slectBackX); //获取数组rangeAxisLiOffsetLefts中小于xBm的最大元素值
				function offTomap(xun) {
					var off = Range.floorArr(vis.left, xun);
					var x1 = off.val,
